...
extern "C" fn did_finish_launching(this: &Object, _: Sel, _: id) {
  ...
  // macOS 26+ patch: catch any panics in launched() to prevent them from
  // crossing the FFI boundary, which causes `panic_cannot_unwind` + SIGABRT.
  ...
  .is_err()
  {
    error!("Panic in applicationDidFinishLaunching — caught to prevent SIGABRT");
  }
